%\usepackage[9pt]{extsizes}

\usepackage[utf8]{inputenc} % Required for including letters with accents
\usepackage{multicol}

\usepackage{marginnote}

\usepackage{wrapfig}

\newmdenv[skipabove=7pt,
skipbelow=7pt,
rightline=false,
leftline=true,
topline=false,
bottomline=false,
backgroundcolor=ocre!10,
linecolor=ocre,
innerleftmargin=8pt,
innerrightmargin=8pt,
innertopmargin=8pt,
innerbottommargin=8pt,
leftmargin=0cm,
rightmargin=0cm,
linewidth=4pt]{siamtitle}	

\setlength{\columnseprule}{0.5pt}
\def\columnseprulecolor{\color{gray}}

\newcommand{\mail}[1]{\href{mailto:#1}{\texttt{#1}}}
\newcommand{\member}[2]{{\textbf{#1}\\}{#2}}

% with new page
%\newcommand{\siamsessiontitle}[2]{\newpage\begin{siamtitle}\noindent{\large{\textbf{{#1}  {#2}}}}\end{siamtitle}}
\newcommand{\siamsessiontitle}[2]{\begin{siamtitle}\noindent{\large{\textbf{{#1} {#2}}}}\end{siamtitle}}
\newcommand{\siamsessiondate}[1]{\noindent\textbf{#1}\\}
\newcommand{\siamsessionroom}[1]{\noindent\textbf{#1}}
\newcommand{\siamsessionabstract}[1]{\medskip{#1} \medskip}


\newcommand{\siamschedule}[3]{{\contentslabel[\colorbox{blue!30}{\large{\textbf{#1}}}]{2cm}}{\textbf{#2}\\\indent{#3}\medskip}}
%\newcommand{\siamschedule}[3]{{\marginnote{#1}[2cm]}\noindent\textbf{#2}\\ {#3}\medskip}
\newcommand{\siampresentationtitle}[1]{\noindent\textbf{#1}}
\newcommand{\siampresentationabstract}[1]{\noindent{#1}}

\newcommand{\siamauthor}[2]{{\color{blue}{{#1}}}\\\textit{#2}}

\newcommand{\siamscheduleday}[1]{\newpage\rhead{Program for #1}\begin{center}\noindent\textbf{\large{#1}}\par\noindent\rule{\textwidth}{0.4pt}\end{center}}

\newenvironment{organizers}{\begin{flushright}\textbf{Organizers:} \\}{\end{flushright}}

% who columns as 2016
\newenvironment{siamsessions}{\begin{multicols}{2}\raggedcolumns}{\end{multicols}}
\newenvironment{presentation_list}{}{}
%\newenvironment{presentation_list}{}{\vfill\null \columnbreak}
% standard
%\newenvironment{siamsessions}{}{}
%\newenvironment{presentation_list}{\begin{multicols}{2}}{\end{multicols}}

\newenvironment{siamppresentationwithhour}[2]{\noindent\textbf{{#1} {#2}}}

\DeclareUnicodeCharacter{3B1}{\ensuremath{\alpha}}

\newcommand{\R}{\mathbb{R}} 

\pagestyle{fancy}
\fancyhf{}


