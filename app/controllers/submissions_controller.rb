class SubmissionsController < ApplicationController
  def index
    if user_in_organizer_commettee?
      @minisymposia  = Minisymposium.submitted.includes(ratings: :user, organizers: :user).all
      @presentations = Presentation.unassigned.submitted.includes(ratings: :user, authors: :user).not_poster
      @posters       = Presentation.submitted.includes(ratings: :user, authors: :user).poster
    else
